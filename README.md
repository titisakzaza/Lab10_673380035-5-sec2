...
├── repository/
│   └── ProductRepository.java     ← ✅ Reactive in-memory repository
├── service/
│   └── ProductService.java        ← ✅ Reactive business logic
├── controller/
│   └── ProductController.java     ← ✅ getById ทำแล้ว / ❌ TODO: อีก 5 endpoints
└── client/
    └── ProductWebClient.java       ← ✅ WebClient methods ครบทุก endpoint
```
...
| GET | `/products` | `Flux<Product>` | ✅ เสร็จแล้ว |
| POST | `/products` | `Mono<Product>` | ✅ เสร็จแล้ว |
| DELETE | `/products/{id}` | `Mono<Void>` | ✅ เสร็จแล้ว |
| GET | `/products/category/{cat}` | `Flux<Product>` | ✅ เสร็จแล้ว |
| GET | `/products/{id}/price` | `Mono<Double>` | ✅ เสร็จแล้ว |
...
```bash
# Clone repo
git clone https://github.com/<your-username>/lab10-673380035-5-sec2.git
cd lab10-673380035-5-sec2
...
- [x] **GitHub Repository** — ชื่อ `lab10-673380035-5-sec2` พร้อม commit history
- [x] **Code ครบทุก TODO** — Repository, Service, Controller, WebClient
- [x] **ผลลัพธ์ใน response มีชื่อและรหัสนักศึกษา** (ใน Product name ที่ seed ไว้)
